$PBExportHeader$n_rmt_menu.sru
$PBExportComments$Menu service
forward
global type n_rmt_menu from n_rmt_baseobject
end type
end forward

global type n_rmt_menu from n_rmt_baseobject autoinstantiate
end type
...
this.of_GetMDIFrame(am_source, lw_frame)   
if IsValid (lw_frame) then
	// Try sending the message to the active MDI sheet
	lw_sheet = lw_frame.GetActiveSheet()
	if IsValid (lw_sheet) then
		li_result = lw_sheet.dynamic event ue_messagerouter(as_message)
	end if

	if li_result <> 1 then
		// Try sending the message to the frame
		li_result = lw_frame.dynamic event ue_messagerouter(as_message)
	end if
else
	// Try sending the message to the parentwindow
	if IsValid (am_source.parentwindow) then
		li_result = am_source.parentwindow.dynamic event ue_messagerouter(as_message)
	end if
end if
...
//if li_result = -1 and IsValid (gnv_app.inv_debug) then
...
on n_rmt_menu.create
TriggerEvent( this, "constructor" )
end on

on n_rmt_menu.destroy
