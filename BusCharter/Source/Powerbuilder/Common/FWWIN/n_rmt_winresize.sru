$PBExportHeader$n_rmt_winresize.sru
$PBExportComments$Object for resizing all visual objects
forward
global type n_rmt_winresize from n_rmt_baseobject
end type
end forward
...
global n_rmt_winresize n_rmt_winresize
...
public function integer of_register (powerobject apo_control[], string as_scale)
end prototypes
...
//
//	Arguments:		
//	apo_control		The parent object's control array
//	as_method		The desired resize/move method.
//						Valid values are:
//						 'FixedToRight'
//						 'FixedToBottom'
...
//						1 if it succeeds and -1 if an error occurs.
//
//	Description:  	Register all controls of the parent which needs to either be 
//						moved or resized when the parent object is resized. 
//
//////////////////////////////////////////////////////////////////////////////
//
//	Revision History
...
int   li_totalcontrols
int   li_ndx
int   li_result
dragobject  ldo

li_totalcontrols = upperbound(apo_control)
for li_ndx = 1 to li_totalcontrols
	li_result = this.of_register(apo_control[li_ndx], as_scale)
	if li_result = -1 then
		continue
	end if
	ldo = apo_control[li_ndx]
	choose case ldo.typeof() 
		case datawindow!, dropdownlistbox!, dropdownpicturelistbox!, & 
			  treeview!, userobject!, listview!, multilineedit!, tab!
			continue
			
		case else
         this.of_setminsize(ldo.width, ldo.height)
	end choose
next
...
on n_rmt_winresize.create
TriggerEvent( this, "constructor" )
end on

on n_rmt_winresize.destroy
TriggerEvent( this, "destructor" )
end on

event constructor;//////////////////////////////////////////////////////////////////////////////
//
//	Event:  ue_n_cst_resize
//
//	Description:
//	Resize service.
//
