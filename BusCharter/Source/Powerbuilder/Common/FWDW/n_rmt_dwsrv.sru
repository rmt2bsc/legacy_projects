...
public function integer of_protectdwcolumns (datawindow adw_obj, string as_column_name, boolean ab_protect)
end prototypes
...
//	Returns:  integer
//
//	Description: Changes the color and protect properties of a selected control of 
//              the selected datawindow object base on ab_updatemode.   If
//              ab_protect is true, then background color of columns are 
//              changed to grey and will appear as protected.  Otherwise,
//              the background is changed to white and will appear as 
//              unprotected.
//
//////////////////////////////////////////////////////////////////////////////
//	
...
ls_name = adw_obj.describe(as_column_name + ".name")
ls_tabseq = adw_obj.describe(as_column_name + ".tabsequence")
ls_style = lower(adw_obj.describe(as_column_name + ".edit.style"))
if ls_tabseq = "0" then
	return 0
end if
if ls_style = "editmask" then
	if ab_protect then
	   ls_result = adw_obj.modify(as_column_name + ".editmask.spin=no")
	else
		ls_result = adw_obj.modify(as_column_name + ".editmask.spin=yes")
	end if
end if
if ls_style = "dddw" then
	if ab_protect then
	   ls_result = adw_obj.modify(as_column_name + ".dddw.UseAsBorder=no")
	else
		ls_result = adw_obj.modify(as_column_name + ".dddw.UseAsBorder=Yes")
	end if
end if
ls_result = adw_obj.modify(as_column_name + ".protect='" + string(li_protect) + "'")
if ls_style <> "checkbox" then
  	ls_result = adw_obj.modify(as_column_name + ".background.color=" + string(ll_color) )
end if
