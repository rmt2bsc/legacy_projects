 ...
 *
 */
class VendorPurchasesDatabaseImpl extends XactManagerApiImpl implements VendorPurchasesApi {
     ...
     */
    protected VendorPurchasesDatabaseImpl() throws DatabaseException, SystemException {
	super();
     ...
     * @throws SystemException
     */
    public VendorPurchasesDatabaseImpl(DatabaseConnectionBean dbConn) throws DatabaseException, SystemException {
	super(dbConn);
    }
     ...
     * @throws SystemException
     */
    public VendorPurchasesDatabaseImpl(DatabaseConnectionBean dbConn, Request request) throws DatabaseException, SystemException {
	this(dbConn);
     ...
     * @throws SystemException
     */
    public VendorPurchasesDatabaseImpl(DatabaseConnectionBean dbConn, Request request, Response response) throws DatabaseException, SystemException {
	this(dbConn, request);
    }
     ...
     * @throws VendorPurchasesException
     */
    public List<VwVendorItemPurchaseOrderItem> findVendorItemPurchaseOrderItems(int vendorId, int poId) throws VendorPurchasesException {
     ...
     * @throws VendorPurchasesException
     */
    public List<PurchaseOrderItems> findPurchaseOrderItems(int poId) throws VendorPurchasesException {
	PurchaseOrderItems obj = VendorPurchasesFactory.createPurchaseOrderItem();
	obj.addCriteria(PurchaseOrderItems.PROP_POID, poId);
	...
	// Build list business id's to submit to web service
        ...
        BusinessContactCriteria busIdList = f.createBusinessContactCriteria();
        busIdList.getBusinessId().add(BigInteger.valueOf(po.getBusinessId()));

	// Call web service to fetch business contact based on list of business id's
        RSBusinessContactSearch resp = null;
        try {
            ...
            resp = jaxb.getBusinessContactData(busIdList, userSession.getLoginId());
        }
        catch (Exception e) {
            throw new VendorPurchasesException(e);
        }
	PurchaseOrderExt poExt = this.populatePurchaseOrderExt(po, resp.getBusinessList());
	return poExt;
    }
     ...
     * @throws VendorPurchasesException
     */
    public List <PurchaseOrderExt> findPurchaseOrder(String criteria) throws VendorPurchasesException {
	...
	try {
	    ...
	    if (poList == null) {
		poList = new ArrayList<VwPurchaseOrderList>();
	    }
	}
	catch (Exception e) {
	    throw new VendorPurchasesException(e);
	...
	// If zero purchase orders are found based on selection criteria, do not try to 
	// obtain business contacts since it is an expensive opertaion.  Just return an
	// initialized instance of List<PurchaseOrderExt>.
	if (poList.isEmpty()) {
	    return new ArrayList<PurchaseOrderExt>();
	}
	// Build list business id's to submit to remote service
	ObjectFactory f = new ObjectFactory();
	// Build list of business id's        
	BusinessContactCriteria busIdList = f.createBusinessContactCriteria();
	for (VwPurchaseOrderList obj : poList) {
	    busIdList.getBusinessId().add(BigInteger.valueOf(obj.getBusinessId()));
	}
	...
	// Populate PurcaseOrderExt instance with PO and Business Contact data
     ...
     * @throws VendorPurchasesException
     */
    private PurchaseOrderExt populatePurchaseOrderExt(VwPurchaseOrderList po, List<BusinessType> busList) throws VendorPurchasesException {
	PurchaseOrderExt poExt = new PurchaseOrderExt();
	
	for (BusinessType bt : busList) {
	    // Get business contact data.
	    try {
                if (bt.getBusinessId().intValue() != po.getBusinessId()) {
                    continue;
                }
                
                poExt.setBusinessId(bt.getBusinessId().intValue());
            }
            catch (Exception e) {
                poExt.setBusType(0);
            }
            // Get entity type
            try {
                poExt.setBusType(bt.getEntityType().getCodeId().intValue());
            }
            catch (Exception e) {
                poExt.setBusType(0);
            }
            // Get service type
            try {
                poExt.setServType(bt.getServiceType().getCodeId().intValue());
            }
            catch (Exception e) {
                poExt.setServType(0);
            }
            poExt.setName(bt.getLongName());
            poExt.setShortname(bt.getShortName());
            poExt.setContactFirstname(bt.getContactFirstname());
            poExt.setContactLastname(bt.getContactLastname());
            poExt.setContactPhone(bt.getContactPhone());
            poExt.setContactExt(bt.getContactExt());
            poExt.setTaxId(bt.getTaxId());
            poExt.setWebsite(bt.getWebsite());
            break;
	}
...
//	}
     ...
     * @throws VendorPurchasesException
     */
    public List<PurchaseOrderStatusHist> findPurchaseOrderHistory(int poId) throws VendorPurchasesException {
	PurchaseOrderStatusHist obj = VendorPurchasesFactory.createPurchaseOrderStatusHist();
	obj.addCriteria(PurchaseOrderStatusHist.PROP_POID, poId);
	try {
	    return this.daoHelper.retrieveList(obj);
	...
	try {
	    ...
	    cred = this.getVendor(po.getCreditorId());
     ...
     * @throws VendorPurchasesException
     */
    private Creditor getVendor(int vendorId) throws VendorPurchasesException {
	CreditorApi credApi = null;
	...
	try {
	    ...
	    vendor = (Creditor) credApi.findById(vendorId);
	    return vendor;
	}
	catch (Exception e) {
	    throw new VendorPurchasesException(e);
    ...
    /**
     * Masks the vendor's external account number details except the last four digits and assigns the results to the xact instance. 
     * 
	...
	try {
	    Creditor creditor = (Creditor) credApi.findById(this.vendorId);
	    if (creditor == null) {
		this.msg = "Unable to create vendor purchase order transction due to vendor's profile is not found in the database using creditor id: " + this.vendorId;
	    ...
	    this.msg = "Unable to create vendor purchase order transction due to the occurrence of a database error while attempting to fetch vendor's profile from the database using creditor id: " + this.vendorId;
	    logger.error(this.msg);
