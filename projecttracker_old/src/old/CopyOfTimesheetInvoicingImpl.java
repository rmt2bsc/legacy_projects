 ...
 *
 */
class CopyOfTimesheetInvoicingImpl extends RdbmsDataSourceImpl implements InvoicingApi {
     ...
     */
    protected CopyOfTimesheetInvoicingImpl() throws DatabaseException, SystemException {
	super();
    }
     ...
     * @throws SystemException
     */
    public CopyOfTimesheetInvoicingImpl(DatabaseConnectionBean dbConn, Request request) throws DatabaseException, SystemException {
	super(dbConn);
    ...
    /**
     * Create a CopyOfTimesheetInvoicingImpl object which will contain a database connection, the 
     ...
     * @throws SystemException
     */
    public CopyOfTimesheetInvoicingImpl(DatabaseConnectionBean dbConn, Request request, Response response) throws DatabaseException, SystemException {
	this(dbConn, request);
	this.response = response;
    }

    /**
     * Initializes the necessary api's needed make a CopyOfTimesheetInvoicingImpl functional.  The 
    ...
    public void init() {
	super.init();
	logger = Logger.getLogger("CopyOfTimesheetInvoicingImpl");
     ...
     * @throws InvoicingException
     */
    private String buildSalesOrder(ProjClient client, ProjTimesheet timesheets[]) throws InvoicingException {
	...
	try {
	    ...
	    List ts = this.daoHelper.retrieveList(obj);
	    if (ts == null) {
		return 0;
	    }
	...
	int xactId;
	try {
	    String xml = this.buildSalesOrder(client, timesheets);
	    String results = this.invoiceSalesOrder(xml);
	    ...
	    xactId = rc.getCode();
	    this.postInvoice(timesheets, xactId);
	    return xactId;
	}
	catch (Exception e) {
	    throw new InvoicingException(e);
     ...
     */
    private String invoiceSalesOrder(String salesOrder) throws DatabaseException, InvoicingException {
	// Call service to get customer by client id.
	this.request.setAttribute(AbstractExternalServerAction.ARG_PAYLOAD, salesOrder);
	HttpRemoteServicesConsumer srvc = new HttpRemoteServicesConsumer(this.request, this.response, "invoiceNewSalesOrder");
	srvc.processRequest();
