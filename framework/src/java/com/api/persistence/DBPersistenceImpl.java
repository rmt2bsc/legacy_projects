...
public class DBPersistenceImpl extends RMT2Base implements PersistenceManager {

    private final static Logger logger = Logger.getLogger(DBPersistenceImpl.class);
     ...
     * Default Constructor
     */
    public DBPersistenceImpl() throws Exception {
	return;
    }
    
    /**
     * Create DBPersistenceImpl object with the fiel path of the properties file used to load the API's configuration.
     * 
     * @param config
     * @throws Exception
     */
    public DBPersistenceImpl(String propertiesFilename) throws Exception {
    ...
    /**
     * Create DBPersistenceImpl object with a valid instance of InputStream which represents a loaded 
     * configuration for this API implementation.
     * 
     * @param input
     * @exception Exception
     */
    public DBPersistenceImpl(InputStream input) throws Exception {
	init(input);
    }
     ...
     * 
     * 
     * @param propertiesFilename absolute filepath or classpath of the properties file
     * @exception Exception;
     */
    public void init(Object config) throws Exception {
	if (config == null || !(config instanceof String)) {
	    this.msg = "Unable to initialize the PersistenceManager API using DBPersistenceImpl due to its input configuration is invalid or null.   The configuration must be an instance of a String representing the file path or classpath to the API's configuration.";
	    logger.error(this.msg);
	    throw new Exception(this.msg);
	}
	String propertiesFilename = config.toString();
	...
	try {
	    home_dir = home_dir + '/' + propertiesFilename;
	...
	try {
	    InputStream in = DBPersistenceImpl.class.getResourceAsStream('/' + propertiesFilename);
	    if (in != null) {
		init(in);
		return;
	    }
		...
		log.error("failed reading database properties from " + propertiesFilename + ", exception=" + x);
	}
	...
	throw new Exception("DBPersistenceManager.DBPersistenceManager(): " + "failed reading database properties from " + propertiesFilename);
    }
	...
	if (!entryExists(key)) {
		...
		t1.printStackTrace();
		//trace here
		throw new CannotPersistException(" error adding a completely new entry in to DB ", t1);
	    }
	}// checking entries
	...
	try {
	    ...
	    throw new CannotPersistException("error updating an existing entry in to the database ", t);
	}
	// cleanup
	finally {
	    try {
	...
	try {
	    ...
	    //trace
	    t3.printStackTrace();
	    throw new CannotRemoveException(" Error retrieving value for given key", t3);
	}
	finally {
	    try {
		if (prepStat != null)
	...
	try {
	    ...
	    t.printStackTrace();
	    // throw Exception
	    throw new CannotRemoveException("Could not remove existing entry due to error in jdbc transaction", t);
	}
	...
	try {
	    ...
	    t.printStackTrace();
	    //trace here
	    throw new CannotPersistException("Error with the map entered to saveAll", t);
	}
	...
	try {
	    ...
	    //trace here
	    throw new CannotRetrieveException("Error happened while querying the database for bulk retrieve, try starting DB manually", t);
	}
	...
	try {
	    ...
	    //trace here
	    throw new CannotRemoveException(" delete all query failed with existing database", t);
	}
	...
	try {
	    ...
	    t.printStackTrace();
	    //trace here
	    throw new CannotPersistException("error adding new entry using creating Db connection and schema", t);
	}
    }// end of addentry..
	...
	try {
		...
		log.info("userName=" + userName + ", userPass=" + userPass + ", connStr=" + connStr);
	    return conn;
	}
	catch (Throwable t) {
	    t.printStackTrace();
	    //trace here
	    throw new CannotConnectException("Error in creating connection using provided properties ", t);
	}
    }// end of get conn..
	...
	try {
	    ...
	    //trace here..
	    e.printStackTrace();
	    throw new CannotConnectException("there was an error in creating statements for persisting data using created connection", e);
	}
	try {
	...
	try {
	    ...
	    t.printStackTrace();
	    // trace here
	    throw new CannotCreateSchemaException("error was using schema with blobs", t);
	}// end of catch
