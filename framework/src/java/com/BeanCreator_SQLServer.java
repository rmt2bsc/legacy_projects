...
public class BeanCreator_SQLServer {
    ...
    public BeanCreator_SQLServer() {

        try {
            Properties dbParms = new Properties();
            prop = ResourceBundle.getBundle(HttpSystemPropertyConfig.CONFIG_SYSTEM);

            this.packageName = prop.getString("orm_bean_package");
            if (this.packageName == null) {
                this.packageName = "com.bean";
            }
    ...
    public void produceBeans() {
        ...
        try {
            ...
            rsSysTable = stmt.executeQuery("select name \"table_name\", id \"table_id\" from sysobjects where type in ('U', 'V')");
            ...
            while (rsSysTable.next()) {
                ...
                this.header.append("package " + this.packageName + ";\n\n\n");
                ...
                int result = this.buildBeanAttributes(tableId);
    ...
    public void start() {
        try {
            BeanCreator_SQLServer bean = new BeanCreator_SQLServer();
            bean.produceBeans();
        }
        catch (Exception e) {
            System.out.println(e.getMessage());
