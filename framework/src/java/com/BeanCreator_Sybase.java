...
public class BeanCreator_Sybase {
    ...
    public void produceBeans() {
        ...
        try {
            ...
            sql = "select table_name, st.table_id from sysobject so, systable st where so.object_id = st.object_id and so.object_type in (1, 2) and st.creator in (select user_id from sysuser where user_name = "
                    + user + ")";
            ...
            while (rsSysTable.next()) {
                ...
                String packageLoc = RMT2File.getPropertyValue(RMT2Utility.CONFIG_SYSTEM, "orm_bean_package");
