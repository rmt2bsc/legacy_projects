/**
 * Renders various HTML form elements using XML as the input source.  
 * This library depends on AjaxRequestConfig.js and xpath.js libraries.
 */
function AjaxRenderer(reqConfig, xmlData) {
	...
	/**
	 * Retrieve data value for a XML tag element using XPath expressions.   The value 
	 * is returned as an array in the event the propName occurs multiple times.
	 */
	 ...
	 * data derived from the user's Ajax request.
	 */
	this.buildSelectOptions = function(selObj, codeName, textName) {
	    var codes;
	    var text;
			codes = this.getElementValue(codeName);
	    text = this.getElementValue(textName);
	...
	this.renderHtml = function () {
		...
		if (config.component.type == "select-one" || config.component.type == "select-multiple") {
			renderSelect();
		}
	}
	...
	var validate = function () {
		...
		if (config.component.type != "text" && config.component.type != "checkbox" && config.component.type != "radio" && config.component.type != "textarea" && config.component.type != "select-one" && config.component.type != "select-multiple") {
			return false;
		}
		return true;
	}
