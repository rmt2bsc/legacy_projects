...
import com.services.UsersConsumer;
 ...
 * 
 */
public class TimesheetListAction extends AbstractActionHandler implements ICommand {
    private static final String COMMAND_ADD = "Project.TimesheetList.add";

    private static final String COMMAND_EDIT = "Project.TimesheetList.edit";

    private static final String COMMAND_LIST = "Project.TimesheetList.list";

    private static final String COMMAND_SEARCH = "Project.TimesheetList.search";

    private static final String COMMAND_NEWSEARCH = "Project.TimesheetList.newsearch";

    private static final String COMMAND_OLDSEARCH = "Project.TimesheetList.oldsearch";

    private static final String COMMAND_BULKINVOICE = "Project.TimesheetList.bulkinvoice";
     ...
     * 
     */
    public TimesheetListAction() {
	logger = Logger.getLogger("TimesheetListAction");
    }
	...
	try {
	    ...
	    this.empApi = ProjectManagerFactory.createEmployeeApi(this.dbConn, request);
	    ...
	    if (!command.equalsIgnoreCase(TimesheetListAction.COMMAND_NEWSEARCH)) {
		this.initMode();
	    }

	    // Get employee that is currently logged in
	    try {
		this.emp = this.empApi.getLoggedInEmployee();
	    }
	    ...
	    else if (command.equalsIgnoreCase(TimesheetListAction.COMMAND_ADD)) {
		this.addData();
	    }
	    else if (command.equalsIgnoreCase(TimesheetListAction.COMMAND_EDIT)) {
		this.editData();
	    }
	    else if (command.equalsIgnoreCase(TimesheetListAction.COMMAND_SEARCH)) {
		this.doSearch();
	    }
	    else if (command.equalsIgnoreCase(TimesheetListAction.COMMAND_OLDSEARCH)) {
		this.searchMode = true;
		this.getTimesheetList();
	    }
	    else if (command.equalsIgnoreCase(TimesheetListAction.COMMAND_LIST)) {
		this.searchMode = true;
		this.getTimesheetList();
	    }
	    else if (command.equalsIgnoreCase(TimesheetListAction.COMMAND_BULKINVOICE)) {
		this.showClientInvoiceList();
	    }
	}
	catch (Exception e) {
	...
	try {
	    weekDates = RMT2Utility.getWeekDates(new java.util.Date());
	    // Get ending period date value for the first and second relational
	    // expressions
	    endPeriod2 = RMT2Utility.formatDate(weekDates[6], "MM/dd/yyyy");
	    ...
	    endPeriod1 = RMT2Utility.formatDate(tempCal.getTime(), "MM/dd/yyyy");
	...
	try {
	    ...
	    // this call.
	    TimesheetCriteria criteriaObj = query.getCustomObj() == null ? new TimesheetCriteria() : (TimesheetCriteria) query.getCustomObj();
	    String temp = this.request.getParameter("qry_ProjEmployeeId");
	    criteriaObj.setQry_ProjEmployeeId(String.valueOf(temp));
	    query.setCustomObj(criteriaObj);
		...
		sql = " proj_employee_id = " + this.emp.getEmployeeId();
		criteriaObj.setQry_ProjEmployeeId(String.valueOf(String.valueOf(this.emp.getEmployeeId())));
	    }
	    return sql;
	}
	catch (Exception e) {
	...
	if (orderBy == null) {
	    orderBy = " end_period desc, timesheet_id desc ";
	    this.query.setOrderByClause(orderBy);
	}

	try {
	    // Get Timesheets
	    this.api.setBaseClass("com.bean.VwTimesheetList");
	    this.api.setBaseView("VwTimesheetListView");
	    this.timesheets = this.api.findData(criteria, orderBy);

	    // Get employees
	    this.api.setBaseClass("com.bean.VwEmployeeExt");
	    this.api.setBaseView("VwEmployeeExtView");
	    if (this.mode.equalsIgnoreCase(ProjectConst.TIMESHEET_MODE_MANAGER)) {
		...
		emplCriteria = "login_id = '" + userSession.getLoginId() + "'";
	    }
	    this.employees = this.api.findData(emplCriteria, " shortname asc ");

	    // Get clients
	    this.api.setBaseClass("com.bean.VwCustomerName");
	    this.api.setBaseView("VwCustomerNameView");
	    this.clients = this.api.findData(null, " name asc ");

	    this.api.setBaseClass("com.bean.ProjTimesheetStatus");
	    this.api.setBaseView("ProjTimesheetStatusView");
	    this.status = this.api.findData(null, " id asc ");
     ...
     * </tr>
     * <tr>
     * <td>ArrayList of {@link VwTimesheetList}</td>
     * <td>timesheets</td>
     * </tr>
     * <tr>
     * <td>{@link VwCustomerName}</td>
     * <td>clients</td>
     * </tr>
     * <tr>
	...
	if (this.searchMode) {
		...
		// Prepare to add timesheet.
		List list = this.getClientList();
		String endingPeriodDates[] = this.getEndingPeriods();
		this.request.setAttribute(ProjectConst.CLIENT_DATA_CLIENTS, list);
	...
	try {
	    this.api.setBaseClass("com.bean.VwCustomerName");
	    this.api.setBaseView("VwCustomerNameView");
	    list = this.api.findData(null, orderBy);
	    return list;
	}
	catch (SystemException e) {
	    throw new ActionHandlerException(e.getMessage());
	...
	endingPeriodDates = this.tsApi.getAvailableEndingPeriods();
     ...
     * 
     */
    protected void showClientInvoiceList() {
