...
import com.services.PostalSearchCriteriaException;
 ...
 *
 */
public class PostalSearchHandler extends CommonMessageHandlerImpl {

    private static Logger logger = Logger.getLogger(PostalSearchHandler.class);
     ...
     * @param request
     */
    public PostalSearchHandler(DatabaseConnectionBean con, Request request) {
	super(con, request);
    }
	...
	String xml = null;
	this.reqMessage = (RQPostalSearch) parms.get(SoapProductBuilder.PAYLOADINSTANCE);
	...
	try {
	    ...
	    this.msg = "Postal Search handler failed due to a search criteria validation error";
	...
	if (this.reqMessage.getPostalCriteria().getIpAddr() != null) {
		...
		this.msg = "Unable to execute Postal Search Service due to IP Address fetch error";
		logger.error(this.msg);
		throw new MessagingHandlerException(this.msg, e); 
	    }
	}
     ...
     */
    protected String doFetchIpAddressDetails(IpCriteriaType criteria) throws IpException {
